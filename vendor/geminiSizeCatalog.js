/**
 * ClearNano — Vendored upstream watermark size/config catalog.
 *
 ...
 *   2. Replace the body of this file with that source, verbatim.
 *   3. Re-apply ONLY these two mechanical adaptations (upstream is an ESM
 *      module, this file is a plain <script>, no build step in ClearNano):
 *        a) Remove the `export` keyword in front of each
 *           `export function ...` declaration (keep the function body as-is).
 *        b) Replace the trailing `export { OFFICIAL_GEMINI_IMAGE_SIZES };`
 *           with the `window.GeminiSizeCatalog = { ... }` block at the
 *           bottom of this file (update the property list if upstream
 *           renamed/added/removed any exported function).
 *   4. Update the "Upstream version synced" comment above.
 *   5. Do NOT hand-edit any watermark rule/config logic in this file —
 *      any fix belongs upstream. app.js only maps this catalog's
 *      {logoSize, marginRight, marginBottom, alphaVariant} shape onto
 *      ClearNano's own {size, margin, maskKey} mask lookup — see
 *      `getWatermarkConfig` / `detectBestConfig` / `mapCatalogConfigToMask`
 *      in app.js. That mapping code should rarely need to change.
 */
...
function resolveOfficialGeminiSearchConfigs(
    width,
    height,
    options = {}
) {
...
// Plain-<script> adaptation (upstream uses ESM `export`): expose the same
// public surface as upstream's module exports via a single namespace global.
window.GeminiSizeCatalog = {
